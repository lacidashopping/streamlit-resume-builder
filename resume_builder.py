...
def create_resume_docx(data):
    ...
    if data['photo']:
        # Create a 1-row, 2-column table to keep photo on the right and contact info on the left
        table = doc.add_table(rows=1, cols=2)
        table.autofit = False
        
        # Set explicitly balanced column widths
        ...
        # Add Contact Information to left cell
        ...
        # Add 2x2 ID Photo to right cell (scaled neatly to 1.5 inches wide)
        p_photo = right_cell.paragraphs[0]
        p_photo.alignment = 2 # Right align cell content
        p_photo.add_run().add_picture(data['photo'], width=Inches(1.5))
        
        # Add spacing after the header block
        doc.add_paragraph()
    else:
        # Standard centered header if no photo is uploaded
    ...
    # Helper function to add structured sections
...
# This initializes our dynamic work experience array tracker
...
for i in range(st.session_state.exp_count):
    ...
    # Append structured entry to array if fields are used
    if role or company:
        experiences.append({"role": role, "company": company, "duration": duration, "description": desc})
    st.markdown("---")

# Dynamic row increment buttons
